...
coordinates <- list(c(8.34234, 48.23424), c(8.34234, 49.23424))

## 1 hour range split into 20 minute intervals
x <- ors_isochrones(coordinates, range = 60*60, interval = 20*60, output = "sf")

values <- sort(unique(x$value))
ranges <- split(x, x$value)
ranges <- ranges[as.character(rev(values))]

names(ranges) <- sprintf("%s min", as.numeric(names(ranges))/60)

mapview(ranges, alpha.regions = 0.2, homebutton = FALSE)
```
...
```{r isochrones_colors}
locations = split(x, x$group_index)

locations <- lapply(locations, function(loc) {
  g <- st_geometry(loc)
  high <- which(loc$value==values[length(values)])
  low <- which(loc$value==values[1])
  g[-low] <- st_sfc(Map(st_difference,
                       g[match(values[-low], loc$value)],
                       g[match(values[-high], loc$value)]))
  st_geometry(loc) <- g
  loc
})

isochrones <- unsplit(locations, x$group_index)
