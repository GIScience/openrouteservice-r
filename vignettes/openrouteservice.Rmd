...
The default response format is geoJSON which allows to easily
[visualize](https://rstudio.github.io/leaflet/json.html#working-with-raw-geojsontopojson)
the results with, e.g. [leaflet](https://CRAN.R-project.org/package=leaflet).
Other output formats, say GPX, can be specified through `format = "gpx"`
argument.
...
The coordinate pairs `c(lng, lat)` can be given as a list, or in a 
...
different bicycle types, walking, hiking, wheelchair and heavy vehicle. These
can be listed with
...
`ors_directions`. For example, setting `elevation = TRUE` adds height to the
coordinates of the points along the route. This information can be then used to
plot the elevation profile colored by steepness obtained by specifying it in
`extra_info`.

```{r elevation_profile, fig.dim=c(10, 5), out.height='100%'}
library(sp)
library(ggplot2)
library(RColorBrewer)

x <- ors_directions(coordinates, profile="cycling-mountain",
                    elevation = TRUE, extra_info = I("steepness"))
...
steepness <- route$properties$extras$steepness$values
