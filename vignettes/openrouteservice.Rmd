...
between given points specified in `coordinates`.
...
The points are specified as a list of coordinate pairs `c(lng, lat)`, or an
object which can be coerced to an interleaved vector of the form `c(lng1, lat1,
lng2, lat2, ...)`. In particular, if the points are provided in rows of a
matrix-like object such as the result of `sf::st_coordinates` it needs to be
transposed.^[This is because R stores matrices in column-major order (c.f.
https://en.wikipedia.org/wiki/Row-_and_column-major_order)]

```{r data_frame_coordinates}
df <- data.frame(lng = c(8.34234, 8.34423), lat = c(48.23424, 48.26424))
...
the route, which can be used to plot the elevation profile colored by steepness
obtained by specifying it in `extra_info`.

```{r elevation, fig.dim=c(10, 5), out.height='100%'}
library(sp)
library(ggplot2)
