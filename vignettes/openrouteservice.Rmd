...
it with e.g. [leaflet](https://CRAN.R-project.org/package=leaflet). Other output
formats, such as GPX, can be specified in the argument `format`.
...
The points are specified as a list of coordinate pairs `c(lng, lat)`, or a
...
Given a point or line geometry one can use `ors_elevation` to query for its
elevation.
